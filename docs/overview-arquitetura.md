...
| Integração bancária Marco 2 | Upload OFX/CSV manual | Waldir exporta do banco, sobe no sistema — custo zero |
| Integração bancária Marco 4 (opcional) | Pluggy Open Finance (opcional) | Automação total se justificado após Marco 2 |
...
## Decisão 2 — Pluggy foi adiado para Marco 4 (Opcional)

**R$2.500/mês é o plano básico do Pluggy** (confirmado em pluggy.ai/pricing). Não há plano mais barato.

Para o Marco 2 (sistema base), o custo não se justifica antes de provar valor. Solução alternativa sem custo adicional:
...
**Pluggy entra no Marco 4 se:** após 2 meses com o sistema funcionando, o upload manual ainda for um ponto de atrito.
...
| Respostas em linguagem natural (Marco 4) | Waldir digita "o Sinijude pagou esse mês?" — recebe resposta direta |
...
Waldir sobe OFX/CSV (ou futuramente Pluggy traz automático)
  ...
  → Marco 3: Gmail API envia e-mail automático para Tiago → Claude processa resposta
  → Marco 4 (opcional): Pluggy identifica pagador diretamente
         │
...
│                                           ▼                        │
│  Marco 4 (opcional): Pluggy Open Finance ─► Upload no sistema              │
└────────────────────────────────────────┬───────────────────────────┘
                                         │
                                         ▼
┌────────────────────────────────────────────────────────────────────┐
...
│  receita/custo│ │                     │ │  (opcional: WhatsApp) │
...
**Marco 3 — Gmail (custo zero, já está no Workspace):**
...
**Opcional — WhatsApp (se Ricardo preferir):**
...
## Consentimento Open Finance (Marco 4 — opcional)
...
| Open Finance: renovação de consentimento esquecida (Marco 4 — opcional) | Alta | Médio | Alertas automáticos 30 e 7 dias antes |
| Nome PIX muito abreviado — fuzzy não resolve | Alta | Médio | Confirmação manual; sistema aprende com o tempo |
| Ricardo Estevam não usa e-mail regularmente | Média | Médio | WhatsApp como alternativa (opcional) |
...
| ZapSign (opcional) | ~R$150 | Assinatura digital de recibos |
| ADV Box API | ~R$280 | Verificar se incluso no plano atual |
| Claude API | ~R$100–300 | Depende do volume; cobrança por uso |
| **Total Marco 2** | **~R$530–780/mês** | Sem Pluggy, sem ZapSign |
| **Total com opcionais** | **~R$800–1.200/mês** | Com ZapSign, Claude, Cloud Run, ADV Box |

> Pluggy (R$2.500/mês) deliberadamente excluído do escopo base. Avaliar no Marco 4 com ROI comprovado.
